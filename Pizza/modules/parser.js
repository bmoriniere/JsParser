    ...
    return new Promise(function(resolve, reject) {
        ...
        lineReader.on('line', (line)=>{
            ...
            if (lineCount === 0) {
                ...
                result.rows[lineCount] = {tomatoesCpt: 0, mushroomsCpt: 0, detail: []};
                _.forEach(otherLineRegexResult, (value, i) => {
                    if(i === 0) {
                        return;
                    }

                    result.rows[lineCount].detail[i - 1] = value;
                    result.cols[i - 1].detail[lineCount] = value;
                    if(value === 'T') {
                        result.tomatoesCpt++;
                        result.rows[lineCount].tomatoesCpt++;
                        result.cols[i - 1].tomatoesCpt++;
                    } else if (value === 'M') {
                        result.mushroomsCpt++;
                        result.rows[lineCount].mushroomsCpt++;
